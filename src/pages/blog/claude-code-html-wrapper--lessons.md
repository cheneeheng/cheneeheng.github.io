...
## Second refusal: the Docker SDK

Node has `dockerode`, it's perfectly good, and I didn't use it. Every Docker interaction in the wrapper is `child_process` running the `docker` CLI:
...
## Third refusal: the permission prompt

This is the one that raises eyebrows. Every container runs Claude like this:

```
docker exec -it -w /workspace <name> claude --dangerously-skip-permissions
```

That flag name is designed to make you nervous, and normally it should. Here it's deliberate, and I made myself write the reasoning into the spec rather than just flipping it. Claude's per-action permission prompt exists to protect your real machine from a tool that can touch your real files. In this setup, Claude runs inside a sandbox whose entire contents are the user's own workspace — the container is the security boundary, and the prompt would be a second lock on a door that's already inside a vault, paid for in friction on every single command.
